...
      
      // FIX: Verificamos el estado HTTP 'ok' que es un indicador más fiable de éxito.
      if (result.ok) {
        Swal.fire({
          icon: "success",
          title: "¡Listo!",
          // Usamos un texto fijo para el éxito ya que sabemos que la actualización ocurrió
          text: "Contraseña actualizada correctamente.", 
          confirmButtonText: "Ir al login"
        }).then(() => navigate("/login"));
      } else {
        // Si result.ok es false (código HTTP 4xx o 5xx), mostramos el mensaje de error del backend.
        Swal.fire("Error", result.message || "No se pudo actualizar la contraseña debido a un error de servidor.", "error");
      }
    } catch (error) {

...
import Product from "../models/Products.js"; // <<<<<< ASEGÚRATE DE QUE ESTE IMPORT EXISTA
...
async function updateProductStock(order) {
    ...
    const updates = order.products.map(async (item) => {
        // Asegúrate de usar el campo de ID correcto de tus productos en la orden
        ...
        try {
            // 1. Decrementar el stock de forma atómica. Usamos $gte para verificar si hay stock.
            const updatedProduct = await Product.findOneAndUpdate(
                ...
                { 
                    $inc: { stock: -quantityToSubtract } // Decrementa el stock
                },
                { new: true }
            );

            if (!updatedProduct) {
                console.error(`❌ [StockUpdate] Producto ${productId} no encontrado o Stock insuficiente antes de la venta.`);
                return;
            }
            
            console.log(`✅ [StockUpdate] Stock de ${updatedProduct.name} actualizado. Nuevo stock: ${updatedProduct.stock}`);

            // 2. Eliminar el producto si el stock resultante es CERO
            ...
            console.error(`🛑 [StockUpdate] Error al procesar stock para producto ${productId}:`, error.message);
        }
    });
